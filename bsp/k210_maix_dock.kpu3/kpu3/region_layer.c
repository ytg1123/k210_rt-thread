...
static int entry_index(region_layer_t *rl, int location, int entry) {
    ...
    int n= location / wh;
    int loc= location % wh;
    return n * wh * (rl->coords + 1 + rl->classes) + entry * wh + loc;
}
...
static void softmax_cpu(region_layer_t *rl, float *input, int n, int batch, int batch_offset,
                        int groups, int stride, float *output) {
    int g, b;
