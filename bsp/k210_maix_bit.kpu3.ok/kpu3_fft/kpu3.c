...
/* 以下ソース中で既に定義されている */
/* fft_lcd_MAiXBIT/main.c */
extern uint32_t g_lcd_gram[38400] __attribute__((aligned(64)));
extern float    hard_power[FFT_N];
extern uint64_t fft_out_data[FFT_N / 2];
extern uint64_t buffer_input[FFT_N];
extern uint64_t buffer_output[FFT_N];
...
uint8_t r,g,b;
    ...
    for(i=0;i<G_AI_HIGHT ;i++){
        for(j=0;j<G_AI_WIDTH ;j++){
         ...
         d3=(b <<11) & 0xF800;
         d = d1|d2|d3;
         //g_fft_gram[i*G_AI_WIDTH+j] = d;
        }
        for(j=G_AI_HIGHT;j<QVGA_HIGHT ;j++){
         //g_fft_gram[i*G_AI_WIDTH+j] = 0;
        }
    }
        g_fft_finish_flag= 1;
...
if(SKIPFFT==1) return;
    for (j=0;j<QVGA_HIGHT;j++){
        for ( i = 0; i < FRAME_1ST / 2; i++)
        {
            input_data = (fft_data_t *)&buffer_input[i];
            //input_data->R1 = g_fft_gram[QVGA_WIDTH/2*j+2*i]>>8;   
            //input_data->R2 = g_fft_gram[QVGA_WIDTH/2*j+2*i+1]>>8;   
            input_data->I1 = 0;
            input_data->I2 = 0;
        }
        for ( i = FRAME_1ST / 2; i < FFT_N / 2; i++)
        {
            input_data = (fft_data_t *)&buffer_input[i];
            input_data->R1 = 0;
            input_data->I1 = 0;
