...
void lcd_init(void)
{
    ...
    rt_kprintf("lcd_init() \n");
    rt_kprintf("tft_hard_init() \n");
    tft_hard_init();
    rt_kprintf("tft_write_command SOFT_RESET \n");
    /*soft reset*/
    ...
    //rt_thread_sleep(1);
    /*pixel format*/
    rt_kprintf("tft_write_command PIXEL_FORMAT_SET \n");
    ...
    /*display on*/
    rt_kprintf("tft_write_command DISPLAY ON \n");
...
void lcd_set_direction(lcd_dir_t dir)
{
    ...
    if (dir & DIR_XY_MASK)
    {
        lcd_ctl.width = BSP_LCD_Y_MAX - 1;
        lcd_ctl.height = BSP_LCD_X_MAX - 1;
    }
    else
    {
        lcd_ctl.width = BSP_LCD_X_MAX - 1;
        lcd_ctl.height = BSP_LCD_Y_MAX - 1;
    }
...
void lcd_clear(uint16_t color)
{
    ...
    tft_fill_data(&data, BSP_LCD_X_MAX * BSP_LCD_Y_MAX / 2);
}
