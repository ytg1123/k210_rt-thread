...
extern void fft_cp2buf(uint32_t *ptr);
#define FFT_N  512U
extern int16_t fft_out_data[FFT_N];
extern int16_t fft_inp_data[FFT_N];
...
int i,j;
    ...
    for(i=0;i<AI_CAM_HIGHT;i++)
        ...
        g_lcd_gram1[i*AI_CAM_WIDTH + j ] = g_lcd_gram0[i*AI_CAM_WIDTH + j    ];
        }
...
void kpu3_main(void *parameter) {
    ...
    dvp_set_ai_addr( (uint32_t)g_ai_buf,                                         /* Red */ 
                     (uint32_t)(g_ai_buf + AI_CAM_WIDTH * AI_CAM_HIGHT),         /* Green */
                     (uint32_t)(g_ai_buf + AI_CAM_WIDTH * AI_CAM_HIGHT * 2));    /* Blue */
...
//    printf("region_layer_init rl1 ok\n");
    ...
    while (1)
    {
        ...
        {
        //fft_cpy_from_data( (uint32_t)&g_lcd_gram0[QVGA_WIDTH*i] );
        //fft_func();

          for(j=0;j< QVGA_WIDTH/2 ;j++){
          //dh = fft_out_data[i*QVGA_WIDTH/2 + j];
          //dl = fft_out_data[i*QVGA_WIDTH/2 + j +1];
          dh = g_lcd_gram0[i*QVGA_WIDTH/2 + j];
          dl = g_lcd_gram0[i*QVGA_WIDTH/2 + j + 1];
          d = (dh << 16) | dl;
          rgbimg[i*QVGA_WIDTH/2 + j] = d;
          }
