...
static int ov5640_mod_reg( u16 reg, u8 mask, u8 val)
{
        ...
        readval = ov5640_rd_reg( reg );
        readval &= ~mask;
        val &= mask;
        val |= readval;
        ov5640_wr_reg( reg, val);
...
int ov5640_set_framefmt( uint32_t code)
{
        ...
        ov5640_wr_reg( OV5640_REG_FORMAT_CONTROL00, val);

        /* FORMAT MUX CONTROL: ISP YUV or RGB */
        ov5640_wr_reg( OV5640_REG_ISP_FORMAT_MUX_CTRL, is_rgb ? 0x01 : 0x00);

        /*
         * TIMING TC REG21:
         * - [5]:       JPEG enable
         */
        ret = ov5640_mod_reg( OV5640_REG_TIMING_TC_REG21, BIT(5), is_jpeg ? BIT(5) : 0);
        if (ret < 0)
                return ret;
        /*
         * SYSTEM RESET02:
         ...
         * - [2]:       Reset JPEG
         */
        ret = ov5640_mod_reg( OV5640_REG_SYS_RESET02, BIT(4) | BIT(3) | BIT(2), is_jpeg ? 0 : (BIT(4) | BIT(3) | BIT(2)));
        if (ret)
                return ret;
         ...
         * - [3]:       Enable JPEG clock
         */
        return ov5640_mod_reg( OV5640_REG_SYS_CLOCK_ENABLE02, BIT(5) | BIT(3), is_jpeg ? (BIT(5) | BIT(3)) : 0);
}
...
uint8_t ov5640_init(void)
{
    ...
    //hal_delay(50);
    //ret = ov5640_set_framefmt( MEDIA_BUS_FMT_RGB565_2X8_BE );
    //if (ret)
    //    return 1;
