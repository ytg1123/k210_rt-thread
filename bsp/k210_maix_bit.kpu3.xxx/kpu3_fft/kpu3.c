...
extern void fft_oneframe(uint16_t *inptr,uint16_t *outptr);
...
/* ramポインタ関連*/
//uint16_t g_lcd_ram[QVGA_WIDTH * QVGA_HIGHT ]     __attribute__((aligned(64))) ;
//uint16_t g_fft_inp_ram[QVGA_WIDTH * QVGA_HIGHT ] __attribute__((aligned(64))) ;
//uint16_t g_fft_out_ram[QVGA_WIDTH * QVGA_HIGHT ] __attribute__((aligned(64))) ;
//uint8_t  g_ai_buf[QVGA_WIDTH * QVGA_HIGHT * 3]   __attribute__((aligned(64))) ;

/* ramポインタ関連*/
uint16_t g_lcd_ram[QVGA_WIDTH * QVGA_HIGHT ];
uint16_t g_fft_inp_ram[QVGA_WIDTH * QVGA_HIGHT ];
uint16_t g_fft_out_ram[QVGA_WIDTH * QVGA_HIGHT ];
uint8_t  g_ai_buf[QVGA_WIDTH * QVGA_HIGHT * 3];
...
uint8_t r,g,b;
    ...
    for(i=0;i<G_AI_HIGHT ;i++){
        for(j=0;j<G_AI_WIDTH ;j++){
         r = g_ai_buf[i*G_AI_WIDTH + j ];
         g = g_ai_buf[i*G_AI_WIDTH + j + G_AI_WIDTH * G_AI_HIGHT     ];
         b = g_ai_buf[i*G_AI_WIDTH + j + G_AI_WIDTH * G_AI_HIGHT * 2 ];
         r = (r>>3) & 0x1f;
         g = (g>>2) & 0x3f;
         b = (b<<3) & 0x1f;
         if(KPU3_DEBUG) rt_kprintf("g_ai_ram r=%2x g=%2x b=%2x \n", r,g,b );
         d1=(r     ) & 0x001f;
         d2=(g << 5) & 0x07e0;
         d3=(b <<11) & 0xF800; 
         d = d1|d2|d3;
         if(KPU3_DEBUG) rt_kprintf("g_ai_ram r|g|b=%4x \n", d );
         //g_fft_inp_ram[i*G_AI_WIDTH+j] = d;
         //g_lcd_ram[i*G_AI_WIDTH+j] = d;
         d = g_lcd_ram[i*G_AI_WIDTH+j] ;
         g_fft_inp_ram[i*G_AI_WIDTH+j] = d;
        }
    }
        g_fft_finish_flag= 1;
...
void kpu3_main(void *parameter) {
    ...
    /* fft buffer clear */
    fft_clr_frame( (uint32_t)g_fft_inp_ram );
    if(KPU3_DEBUG) rt_kprintf("address g_fft_ram[0]=%x \n", (uint32_t)&g_fft_inp_ram[0] );
...
//        region_layer_run(&detect_rl1, NULL);
        ...
        //lcd_draw_picture(0, 0, QVGA_WIDTH, QVGA_HIGHT, (uint32_t)g_lcd_ram);
        lcd_draw_picture(0, 0, QVGA_WIDTH, QVGA_HIGHT, (uint32_t)g_fft_out_ram);
