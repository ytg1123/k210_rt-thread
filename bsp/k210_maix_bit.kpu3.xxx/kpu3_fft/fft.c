...
static uint16_t fft_out_data[FFT_N];
static uint16_t fft_inp_data[FFT_N];
...
/* for fft */
    int i=0,j=0;
     ...
     for (i=0;i<10;i++)
     {
        fft_cpy_from_data( (uint16_t *)&inptr[QVGA_WIDTH*i] );
        //fft_func(     (uint16_t *)&inptr[QVGA_WIDTH*i] ,i );
