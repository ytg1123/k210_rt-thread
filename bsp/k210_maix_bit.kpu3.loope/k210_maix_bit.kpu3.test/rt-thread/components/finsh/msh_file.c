...
/* y.tagashira tests implement mem command */
void cmd_mem(int argc, char** argv)
{
    ...
    if (argc != 3)
    {
        rt_kprintf("Usage: mem address SIZE\n");
    }
    else
    {
    addres =  strtoul(argv[1], NULL, 16);
    length =  strtoul(argv[2], NULL, 16);
    rt_kprintf("addres %08x \n", addres);
    rt_kprintf("length %08x \n", length);
    memcpy(linebuf, (void *)addres, 256);
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_mem, __cmd_mem, dump memory size );
